...
class ParticleSystem extends GameObject {
  ...
  constructor(x, y, color, count, lifeDuration, emitDuration, particleGravity = { x: 0, y: 50 }) {
    ...
    this.particleGravity = particleGravity; // The gravity the particles should be affected by.
  ...
  emitParticles(deltaTime) {
    ...
    for (let i = 0; i < particlesToEmit && this.particlesEmitted < this.count; i++) {
      ...
      // Also adds desired particle gravity.
      const lifeDuration = this.lifeDuration + Math.random() - 0.5;
      const particle = new Particle(this.x, this.y, Math.random() * 5, Math.random() * 5, this.color, lifeDuration, { x: this.particleGravity.x, y: this.particleGravity.y });
      particle.addComponent(new Physics({ x: (Math.random() - 0.5) * 50, y: (Math.random() - 0.5) * 50 }, { x: 0, y: 0 }, { x: 0, y: 0 }));
